...
process GENE_HARMONIZE {
    ...
    if not count_dirs:
        raise RuntimeError("No per-sample counts directories were staged for GENE_HARMONIZE.")
    ...
    def canonicalize_species(value):
        ...
        for canonical, config in species_config.items():
            ...
            if token == canonical or token in alias_tokens:
                return canonical
        raise ValueError(f"Unsupported species label: {value}")

    requested_order = []
    for raw_species in [item for item in "${params.species_order}".split(",") if item.strip()]:
    ...
    def read_count_dir(count_dir):
        ...
        if obs.isnull().all(axis=1).any():
            ...
            raise RuntimeError(f"obs_meta rows did not align to barcodes for {count_dir}: {missing}")

        sample_id = str(obs["sample_id"].iloc[0]) if "sample_id" in obs.columns else count_dir.name.replace("_counts", "")
        ...
        if matrix.shape != (len(barcodes), len(genes)):
            raise RuntimeError(
                f"Matrix shape mismatch for {count_dir}: got {matrix.shape}, expected {(len(barcodes), len(genes))}"
            )
        ...
        obs.index = pd.Index([f"{sample_id}:{barcode}" for barcode in barcodes], name="cell_id")

        var = pd.DataFrame(index=pd.Index([f"feature_{idx}" for idx in range(len(genes))], name="feature_id"))
        var["feature_name"] = genes
        return ad.AnnData(X=matrix, obs=obs, var=var)

    mg = mygene.MyGeneInfo()

    def fetch_species_mapping(species, genes):
        results = mg.querymany(
            list(dict.fromkeys(str(gene) for gene in genes if pd.notna(gene))),
            scopes="symbol",
    ...
    def collapse_to_canonical(adata, mapping_df, shared_gene_set):
        ...
        if not keep_mask.any():
            raise RuntimeError(f"No shared genes remained for sample {adata.obs['sample_id'].iloc[0]}.")
    ...
    def align_features(adata, target_genes):
        ...
        column_positions = np.array([target_lookup[gene] for gene in adata.var_names], dtype=np.int64)
        coo = adata.X.tocoo()
    ...
    if len(present_species) < 2:
        raise RuntimeError("scMODAL integration requires at least two species datasets.")

    ordered_species = [species for species in requested_order if species in present_species]
    ordered_species.extend(sorted(species for species in present_species if species not in ordered_species))
    ...
    for species in ordered_species:
        ...
        mapping_df = fetch_species_mapping(species, all_genes)
    ...
    shared_gene_set = set.intersection(*(canonical_sets[species] for species in ordered_species))
    if not shared_gene_set:
        raise RuntimeError("No shared ortholog-mapped genes were found across the requested species.")
    ...
    for order_index, species in enumerate(ordered_species):
        ...
        dense_x = species_adata.X.toarray() if sparse.issparse(species_adata.X) else np.asarray(species_adata.X)
    ...
    pd.concat(mapping_frames, ignore_index=True).to_csv(output_dir / "ortholog_mapping.csv", index=False)
    pd.DataFrame({"feature_index": range(len(shared_gene_order)), "canonical_gene": shared_gene_order}).to_csv(
        output_dir / "shared_genes.csv", index=False
    )
