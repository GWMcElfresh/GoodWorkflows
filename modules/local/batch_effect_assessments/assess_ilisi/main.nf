process ASSESS_ILISI {
    ...
    output:
    tuple val(meta), val(reduction), path("${meta.id}_${reduction}_ilisi.csv"), emit: metrics

    script:
    def tplDir = "${projectDir}/modules/local/batch_effect_assessments/templates"
    def outCsv = "${meta.id}_${reduction}_ilisi.csv"
    """
    #!/usr/bin/env bash
    set -euo pipefail
    WORK_DIR="\${PWD}"
    ...
    cp "\${TPL}/assess_ilisi.R" "\${WORK_DIR}/.command-metric.R"
