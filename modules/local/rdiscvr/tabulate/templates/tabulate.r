#!/usr/bin/env Rscript
# Nextflow template: tabulate.r
# TABULATE — build a wide, subject-level table from per-sample metadata CSVs.
#
# Nextflow substitutions (all val() process inputs):
#   ${tabulate_id_cols}              – comma-separated ID column names
#   ${tabulate_celltype_cols}        – comma-separated extra cell-type column names (may be empty)
#   ${tabulate_parent_col}           – parent column name for hierarchy filtering (may be empty)
#   ${tabulate_celltype_parent_map}  – colon-delimited key:value pairs, comma-separated (may be empty)
